...
void SGDSolver<Dtype>::ComputeUpdateValue() {
  ...
  case Caffe::CPU:
    for (int param_id = 0; param_id < net_params.size(); ++param_id) {
	  ...
	  if(this->param_.cal_2nd_grad()) {
		  Dtype* param_diff = history_[param_id]->mutable_cpu_data();
		  const Dtype* param_2nd_diff = net_params[param_id]->cpu_2nd_diff();
		  for(int i = 0; i < history_[param_id]->count(); i++)
			  param_diff[i] = param_diff[i] / (abs(param_2nd_diff[i]) + FLT_EPSILON + local_decay);
	  }
      // copy
